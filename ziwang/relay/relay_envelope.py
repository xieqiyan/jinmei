"""Relay application envelope shared by the UDP Relay implementation."""
...
def encode(payload: bytes, ns_ip: str, device: str, reply_port: int,
    ...
    header = HEADER.pack(
        MAGIC, VERSION, 0, DEVICE_CODES[device], 0,
        ipaddress.IPv4Address(ns_ip).packed,
...
def decode(data: bytes) -> Optional[RelayEnvelope]:
    if len(data) < HEADER.size or data[:4] != MAGIC:
        return None
    magic, version, _flags, code, _reserved, address, reply_port, payload_len, request_port = HEADER.unpack_from(data)
    if magic != MAGIC or version != VERSION or code not in CODE_DEVICES:
    ...
    return RelayEnvelope(str(ipaddress.IPv4Address(address)), CODE_DEVICES[code],
                         reply_port, request_port, data[HEADER.size:])
